#define TAKLAMAKAN_DESERT

using CoreTweet;
using KunappCommonRt;
using SDKTemplate;
...
namespace Syndication
{
	...
	/// </summary>
	public sealed partial class S2_Tweet : Page
	{
...
#region Fields
		...
		public S2_Tweet()
		{
			...
			{
				if (res == true)
					_anyMessage.Text = "砂漠に蝶は舞う。";
				else
				{
					_anyMessage.Text = "アプリの使用を許可する。";
		...
		private async void OnAuthorize(object sender, Windows.UI.Xaml.RoutedEventArgs args)
		{
			...
			{
				// URI launched
				_anyMessage.Text = "PINコードを入力するんだ。";
				return;
			}
			else
			{
				...
				await md.ShowAsync();
				_anyMessage.Text = "ジオンの制空権内ということを自覚しろ！";
			}
		}
		...
		private async void PinExeCute(object sender, RoutedEventArgs e)
		{
			...
			{
				_anyMessage.Text = "操作に誤りがあるようだ。聡明な貴方が・・・";
				return;
			}
			if (Tokens != null)
			{
				...
				_anyMessage.Text += "\r ジャブローへの通信回線が開いたぞ。";
			...
			{
				_anyMessage.Text = "ジャブローへの認証が失敗したようだ。貴様何をやっている！";
				return;
			}
		}
		...
		private async void OnTweet(object sender, Windows.UI.Xaml.RoutedEventArgs args)
		{
			...
			{
				_anyMessage.Text = "ミノフスキー濃度が濃いようだ・・・ \r通信回線を調べるんだ。";
				return;
			}
		}
		...
		private async void ShoppingTweet(object sender, RoutedEventArgs e)
		{
			if (Tokens == null)
			{
				_anyMessage.Text = "貴様はシャアを越えられる奴だと思っていた。残念だよ。";
				return;
			}
			string str = _statusTextBox.Text;
			if (str.Length < 1)
			{
				_anyMessage.Text = "商品名入れてないよ！何やってんの！！";
				return;
			}
			if (str.Length > (Const.TweetStringLength - (Const.PreTweetPhrase.Length + Const.PostTweetPhrase.Length)))
			{
				_anyMessage.Text = "文字数が多すぎるぞ！字数は、有効に使え！ (現在の文字数 = " + (str.Length + Const.PreTweetPhrase.Length + Const.PostTweetPhrase.Length).ToString();
				return;
			}
			try
			{
			...
			{
				_anyMessage.Text = "？？ミノフスキー粒子濃度が濃いようだ・・・";
			}
			finally
			{
				progressBar.IsIndeterminate = false;
		...
		private async void LoadTimeLine(object sender, RoutedEventArgs e)
		{
			if (Tokens == null)
			{
				_anyMessage.Text = "ジャブローへの認証が行われていないようだ。";
				return;
			}
			progressBar.Visibility = Visibility.Visible;
			progressBar.IsIndeterminate = true;
			try
			{
				...
				_anyMessage.Text = "肩に力が入りすぎのようだな。大丈夫、コンピュータがやってくれますよ。";
				progressBar.IsIndeterminate = false;
				progressBar.Visibility = Visibility.Collapsed;
			}
			catch (Exception ex)
			{
				_anyMessage.Text = "・・・通信回線に問題があるようだ。第二ブリッジを調べろ。e:" + ex.ToString();
			}
		}
		...
		private async void JetStreamAttack(object sender, RoutedEventArgs e)
		{
			...
			if (n > 0)
				_anyMessage.Text = "システムにトラブル発生。\r\n総員待避！";
			else
				_anyMessage.Text = "全滅だと・・・\r\n機密を消去した。";
		}
